...
Also includes torch fp16 baseline for comparison.

Weight layouts (stored simultaneously by the hybrid kernel):
...
def prepare_hybrid_weights(K, N, group_size, device="cuda"):
    ...
    return w_q_skinny, w_s_skinny, w_q_triton, w_s_triton, w_fp16
    ...
    if provider == "torch-fp16":
        ...
        def run():
            return _hybrid_w4a16_apply_impl(
                ...
                w["w_q_triton"],
                w["w_s_triton"],
                None,  # w_zp_triton (None for symmetric uint4b8)
                None,  # bias
                cu_count,
                group_size,
                8,  # zp_bias for uint4b8
            )
...
if __name__ == "__main__":
    ...
    for K, N, model in prepare_shapes(args):
        ...
        w_q_skinny, w_s_skinny, w_q_triton, w_s_triton, w_fp16 = prepare_hybrid_weights(
            K, N, group_size
        )
